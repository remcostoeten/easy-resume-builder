...
export type TProfessionalSidebarProps = {
  ...
  return (
    <div ref={setNodeRef} style={style}>
      ...
      >
        ...
        <Switch checked={section.isEnabled} onCheckedChange={handleToggle} disabled={section.isRequired} size="sm" />
      </div>
    </div>
  )
}

export function ProfessionalSidebar({ sections, onToggleSection, onReorderSections }: TProfessionalSidebarProps) {
  ...
  return (
    <div className="h-full bg-sidebar border-r">
      <div className="p-4 border-b">
        ...
        <p className="text-xs text-muted-foreground">Reorder sections and toggle visibility</p>
      </div>
          ...
          <SortableContext items={sortedSections.map((s) => s.id)} strategy={verticalListSortingStrategy}>
            <AnimatePresence>
              {sortedSections.map((section) => (
                ...
                >
                  <SortableSectionItem section={section} onToggle={onToggleSection} />
                </motion.div>
              ))}
            </AnimatePresence>
          </SortableContext>
