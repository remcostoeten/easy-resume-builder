...
export function SectionTabs({ sections, activeSection, onSectionChange }: TSectionTabsProps) {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      ...
      switch (event.key) {
        ...
        case "ArrowRight":
          ...
          break
        default:
          // Number keys 1-9 for direct section access
  ...
  }, [currentIndex, enabledSections, onSectionChange, showKeyboardHint])

  // Check scroll state on mount and scroll
  useEffect(() => {
    checkScrollState()
  ...
  useEffect(() => {
    ...
    const activeTab = scrollContainerRef.current.querySelector(`[data-section-id="${activeSection}"]`)
    if (activeTab) {
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      ...
                      >
                        ...
                        <span className="text-xs opacity-60 ml-1">{index + 1}</span>
                    ...
                    <TooltipContent side="bottom" className="text-xs">
                      <div className="text-center">
                        ...
                        <div className="text-muted-foreground">Press {index + 1} or use ← → arrows</div>
                      </div>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
      ...
      {showKeyboardHint && (
        ...
        >
          <div className="bg-popover text-popover-foreground px-3 py-2 rounded-md shadow-lg border text-xs">
            💡 Use ← → arrow keys or number keys (1-{enabledSections.length}) to navigate sections
          </div>
        </motion.div>
      )}
    </div>
