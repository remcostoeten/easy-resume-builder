...
export function extractWorkExperience(text: string): readonly TExtractedWorkItem[] {
	const workSectionRegex =
		/(?:work experience|experience|employment|professional experience)([\s\S]*?)(?:education|skills|projects|$)/i;
...
export function extractEducation(text: string): readonly TExtractedEducationItem[] {
	const educationSectionRegex =
		/(?:education|academic|qualifications)([\s\S]*?)(?:skills|experience|projects|$)/i;
	const educationSection = text.match(educationSectionRegex)?.[1] || '';

	if (!educationSection) return [];

	const educationBlocks = educationSection
		.split(/\n\s*\n/)
...
export function extractSkills(text: string): readonly TExtractedSkillCategory[] {
	const skillsSectionRegex =
		/(?:skills|technologies|technical skills|competencies)([\s\S]*?)(?:education|experience|projects|$)/i;
