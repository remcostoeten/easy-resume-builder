...
import { SkeletonIcon, SkeletonTitle, SkeletonText, SkeletonButton } from './skeleton-factory';

type TSectionType = 'personal-info' | 'work-experience' | 'education' | 'skills' | 'projects' | 'certifications' | 'languages';
...
function ComingSoonSkeleton({ sectionType }: { readonly sectionType: string }) {
	return (
...
export function SectionSkeleton({ sectionType, isLoading = true }: TProps) {
	...
	switch (sectionType) {
		...
		case 'certifications':
		case 'languages':
			return <ComingSoonSkeleton sectionType={sectionType} />;
			
		default:
			return <ComingSoonSkeleton sectionType="unknown" />;
	}
}
